...
courseSchema.methods.saveAndPopTeach = function (res, next) {
  return this.save((errSave, savedCourse) => {
    ...
    this.populate({path: 'teacher locations.votes.user', select: 'email img_url'}, (errPop, populatedCourse) => {
...
courseSchema.statics.findAndPopTeach = function (filter, res, next) {
  return this.find(filter).populate({path: 'teacher locations.votes.user', select: 'email img_url'}).exec((err, populatedCourse) => {
    if (err) next(err)
    res.json(populatedCourse)
  })// end exec
}
...
courseSchema.statics.findSingleAndPopTeach = function (courseId, res, next) {
  return this.findById(courseId).populate({path: 'teacher locations.votes.user', select: 'email img_url'}).exec((err, populatedCourse) => {
    if (err) next(err)
    res.json(populatedCourse)
  })// end exec
}
